...
export class GetRoomUsageTimelineUseCase {
  constructor(private readonly metricRepository: IMetricRepository) {}

  async execute(roomId: string, startDate?: Date, endDate?: Date) {
    ...
    const uniqueStudents = new Set(metrics.map((m) => m.getStudentId()).filter(Boolean));
